...
public:
  ...
  bool HasChanged(void)
  {
    ...
    { // se confirma la transición después de 100 ms
      if (currvalu_ == HIGH && nextvalu_ == LOW)
      { // flanco positivo, se ha pulsado el botón (REVISAR)
        currvalu_ = LOW;
        ms01s1_ = ms01s2_;
        ms01s2_ = msTransit;
      }
      else if (currvalu_ == LOW && nextvalu_ == HIGH)
      {
        ...
        if (currintv == IIBUTTON_INTV_WORD)
        { // poner un espacio antes si estamos empezando una palabra
          strSymbols[iSymbol_++] = ' ';
